...
CMD_CONFIG_REPLACE="wget -O $TMPDIR/replace-config.php https://raw.githubusercontent.com/g105b/github-deploy/master/replace-config.php && DEPLOY_REF=$DEPLOY_REF php $TMPDIR/replace-config.php $DEPLOY_CONFIG_PATH $TMPDIR/$CIRCLE_PROJECT_REPONAME && rm $TMPDIR/replace-config.php"
